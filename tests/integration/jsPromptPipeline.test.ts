...
const fixturePath = path.join(process.cwd(), 'tests', 'integration', 'fixtures', 'config.js');
...
describe('JS file review line mapping', () => {
  ...
  it('applies the inline comment to the intended duplicated JS key line', async () => {
      ...
      () => new TestModelProvider(new TestRuntimeAdapter([
        {
          ...
          message: 'Target duplicated JS key.',
          severity: 'warning',
        },
      ])),
      commentController,
    ...
    expect(appliedThread.uri.fsPath).toBe('config.js');
